...
heimdall_spans
  ...
  kind                  INT                → OpenTelemetry SpanKind
  status                INT   NOT NULL     → 0=UNSET, 1=OK, 2=ERROR
  status_message        TEXT
  start_time_unix_nano  INT   NOT NULL     → Unix nanoseconds (OTel native)
  end_time_unix_nano    INT   NOT NULL
  attributes            JSON               → rpc.method, tool.name, duration.ms, etc.
  events                JSON
  links                 JSON
  resource_attributes   JSON               → service.name, service.version, etc.
...
Same schema as SQLite but with native Postgres types (`BIGINT`, `INTEGER`, `JSON`).
...
Same schema adapted to MySQL types (`BIGINT`, `INT`, `JSON`).
...
Options:
  ...
  --debug               Write verbose logs to stderr
